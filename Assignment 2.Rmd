...
} # NB: perché divide per repetitions ogni volta e somma?? Pensarci!!!!
```
...
fold5_knn <- knn.cv(x = as.matrix(subset(train, select = - diabetes)), y = train$diabetes,
                    nfolds = 5, k = c(1:100), seed = TRUE, stratified = TRUE)
loo_knn <- knn.cv(x = as.matrix(subset(train, select = -diabetes)), y = train$diabetes,
                    nfolds = nrow(train), k = c(1:100), seed = TRUE, stratified = TRUE,
